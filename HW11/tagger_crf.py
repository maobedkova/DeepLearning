...
class Network:
    ...
    def construct(self, args, num_words, num_tags):
        with self.session.graph.as_default():
            ...
            outputs, states = tf.nn.bidirectional_dynamic_rnn(rnn_cell_fwd, rnn_cell_bwd, embedded_word_ids,
            ...
            output = tf.concat(outputs, axis=2, name='concat')
            ...
            log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(output_layer, self.tags,
                                                                                  self.sentence_lens)
            loss = tf.reduce_mean(-log_likelihood)

            # Compute the CRF predictions into `self.predictions` with `crf_decode`.
            self.predictions, best_score = tf.contrib.crf.crf_decode(output_layer, transition_params,
                                                                     self.sentence_lens)
