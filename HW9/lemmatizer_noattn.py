...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            rnn_encoder = tf.nn.rnn_cell.GRUCell(num_units=args.rnn_dim)
            _, source_states = tf.nn.dynamic_rnn(rnn_encoder, embedded_source_seqs, sequence_length=self.source_seq_lens,
                                                 dtype=tf.float32)
