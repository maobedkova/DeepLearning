...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            for argument in args.cnn.split(","):
                ...
                if params[0] == "C":
                    features = tf.layers.conv2d(inputs=features,
                                                ...
                                                strides=[int(params[3])] * 2,
                                                kernel_size=[int(params[2])] * 2,
                                                padding=params[4],
                ...
                # Store result in `features`.
                    ...
                    features = tf.layers.dense(inputs=features,
                                               units=params[1],
                                               activation=tf.nn.relu)
    ...
    def evaluate(self, dataset, images, labels):
        accuracy, _ = self.session.run([self.accuracy, self.summaries[dataset]], {self.images: images,
                                                                                  self.labels: labels,
