    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def encoder(image):
                ...
                hidden_layer = tf.layers.dense(flattened_image, 500, activation=tf.nn.relu)
                ...
                # `z_log_variance`.
                z_mean = tf.layers.dense(hidden_layer, self.z_dim, activation=None)
                z_log_variance = tf.layers.dense(hidden_layer, self.z_dim, activation=None)
            ...
            kl_d = tf.distributions.kl_divergence(tf.distributions.Normal(z_mean, z_sd), tf.distributions.Normal(0.0, 1.0))
            latent_loss = tf.reduce_mean(kl_d)
